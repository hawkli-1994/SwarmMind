...
DEFAULT_CHECKPOINTER_PATH = Path(__file__).resolve().parents[2] / ".runtime" / "deerflow" / "local-default" / "checkpoints.db"
...
class TraceService:
                    ...
                    "tool_calls": [
                        {
                            "id": tc.get("id"),
                            "name": tc.get("name"),
                            "args": self._truncate_content(
                                json.dumps(tc.get("args", {})), 200
                            ),
                        }
    ...
    def _determine_status(self, events: list[dict]) -> str:
        ...
        if event_type == "user_input":
            return "waiting"
        elif event_type in ("assistant_response", "artifact_created"):
            return "completed"
