...
> 关联文档：`docs/architecture.md`、`docs/enterprise-crm-user-story.md`、`docs/mcp-skill-integration.md`
...
```text
AgentTeam
  - default_workflow_template_ref
  - available_workflow_template_refs
```
...
用户在 `Project` 中选择 `AgentTeam` 时，系统应默认加载该 Team 最常用的工作流模板。

也就是说，默认流程应是：

1. 用户选择 `AgentTeam`
2. 系统自动挂载 `default_workflow_template_ref`
3. 用户直接开始描述需求
4. `TeamInterfaceAgent` 基于默认模板启动项目工作流

因此，用户不应被要求先选 Team、再手动重复选择一个几乎同义的模板。

### 5.2 何时需要显式选模板

只有以下情况，才需要让用户显式选择或切换模板：

- 该 Team 绑定了多个可用模板
- 用户明确要切换到非默认流程
- 用户要求基于当前 Team 生成自定义工作流

如果用户没有额外指定，系统应始终先落到默认模板，而不是停在模板选择页等待。
